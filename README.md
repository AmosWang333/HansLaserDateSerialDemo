# 大族激光日期流水号二开 Demo

这是一个 Windows Forms 示例程序，用大族激光 `HansAdvInterface.dll` 二次开发接口把日期流水号写入打标模板中的可变文本对象，并调用设备执行红光预览、激光打标、历史记录查询和历史编号重新打标。

默认产品编号生成器为 `EcoFlow`：`静态前缀 + 年码 + 月码 + 日码 + 4 位流水号`。例如 2026-07-14 第 1 件，在无静态前缀时生成 `J7E0001`。

## 主要功能

- 读取大族激光设备目录中的 `HansAdvInterface.dll`，初始化设备并加载产品模板。
- 在设置窗口维护产品、模板路径、客户料号、Shipcode、生成器、Pattern 和每日起始流水号。
- 支持 `EcoFlow` 和 `Normal` 两种产品级编号生成器。
- 支持在设置窗口读取接口 DLL 版本，便于核对现场环境。
- 打标前先占用编号，只有打标正常结束才提交为 `Marked`。
- 红光预览不提交编号。
- 异常断电、打标失败或退出时保留待确认编号，重启后继续提示处理。
- 支持操作员确认编号已用或跳过，状态记录为 `Skipped`。
- 支持按产品和状态查询历史打标记录，并对历史编号执行重新打标。
- 使用 SQLite 保存产品、流水状态和打标记录。

## 环境要求

- Windows
- .NET 10 SDK
- 大族激光打标控制软件及对应的 `HansAdvInterface.dll`
- 与厂家 DLL 位数一致的编译平台。项目当前配置为 `x86`，如果现场 DLL 明确为 64 位，需要把项目改为 `x64`。

如果运行时出现 `BadImageFormatException`，通常表示程序平台目标与厂家 DLL 位数不一致。

## 模板准备

在大族激光标准打标软件中先准备模板：

1. 完成 BOX 校正、位置校正、激光器参数和图层参数设置。
2. 新建一个文本对象，并设置为可变文本或模型文本。
3. 将可变文本别名设置为 `CODE`，或与 `config.json` 中的 `VariableTextAlias` 保持一致。
4. 保存模板文件，例如 `C:\HansMark\Templates\DateSerial.HS`。
5. 退出标准打标软件和校正软件，避免它们与本程序同时占用设备。

路径建议只使用英文、数字和反斜杠，避免旧版 ANSI 接口无法识别中文路径。

## 设置窗口

点击工具栏中的“设置...”打开设置窗口。窗口包含两个页签：

| 页签 | 用途 |
| --- | --- |
| 运行设置 | 配置设备目录、读取 DLL 版本、可变文本别名、当前产品和脚踏触发参数。 |
| 产品配置 | 新增、删除、编辑产品信息，并保存到 SQLite 数据库。 |

### 运行设置

运行设置字段：

| 字段 | 说明 |
| --- | --- |
| 设备配置目录 | 大族激光打标控制软件目录，目录下必须存在 `HansAdvInterface.dll`。点击“打开”可选择目录。 |
| DLL 版本 | 点击“读取版本”后，通过 `HS_GetDllVersion` 显示所需主程序版本和接口 DLL 版本。 |
| 可变文本别名 | 模板中可变文本对象的别名，默认 `CODE`。 |
| 产品 | 当前要生产的产品。保存并应用后，程序会加载该产品的模板和生成器配置。 |
| 启用脚踏触发 | 是否在激光打标时等待脚踏或外部触发信号。 |
| 等待超时 | 脚踏等待超时时间，界面单位为秒，保存到配置文件时转换为毫秒。范围为 `1` 到 `3600` 秒。 |

点击“保存并应用”后，程序会保存 `config.json`，关闭设置窗口，并按保存后的配置初始化设备、加载所选产品模板。

## 配置文件

程序启动目录下的 `config.json` 会随程序读取和保存。当前保存格式如下：

```json
{
  "MachinePath": "D:\\Han's Laser Marking Control SoftwareV1_1_3",
  "VariableTextAlias": "CODE",
  "UseFootPedal": false,
  "FootPedalTimeoutMs": 600000,
  "ProductId": 1
}
```

字段说明：

| 字段 | 说明 |
| --- | --- |
| `MachinePath` | 大族激光打标控制软件目录，目录下必须存在 `HansAdvInterface.dll`。 |
| `VariableTextAlias` | 模板中可变文本对象的别名，默认 `CODE`。 |
| `UseFootPedal` | 是否启用脚踏或外部触发等待。 |
| `FootPedalTimeoutMs` | 脚踏等待超时时间，单位毫秒。 |
| `ProductId` | 当前选择的产品 ID，由设置窗口保存。 |

产品模板、生成器和 Pattern 不再作为运行设置保存，而是保存在产品表中。旧配置中的 `TemplatePath`、`CodeGeneratorType`、`CodePattern` 字段即使存在，也不作为当前运行依据。

## 产品配置

在“产品配置”页中，产品列表左侧提供新增和删除按钮。选中列表中的产品后，下方“产品信息”区域会载入该产品的详细字段；编辑后点击“保存”写入数据库。

产品字段：

| 字段 | 说明 | 必填 |
| --- | --- | --- |
| `Name` | 产品名称，用于界面显示。 | 是 |
| `CustomerPartNumber` | 客户料号，用于界面显示和查询。 | 否 |
| `Shipcode` | 发运代码。 | 是 |
| `SerialStartValue` | 每天的起始流水号，范围 `1` 到 `9999`。 | 是 |
| `CodeGeneratorType` | 该产品使用的编号生成器，支持 `EcoFlow` 或 `Normal`。 | 是 |
| `TemplatePath` | 该产品使用的打标模板路径。点击“打开”可选择 `.HS` 文件。 | 是 |
| `Pattern` | 编号生成器使用的 Pattern。 | 是 |

删除已保存产品时会弹出确认框。删除产品会同时影响该产品关联的流水状态和历史记录，请按现场数据保留要求操作。

### EcoFlow 生成器

`EcoFlow` 生成格式为：

```text
Pattern + 年码 + 月码 + 日码 + 流水号
```

- 年码当前支持 2025 到 2035。
- 月码和日码使用 `1..9, A..H, J..N, P..X`，跳过 `I` 和 `O`。
- 流水号范围为 `0001` 到 `9999`。
- 产品 `Pattern` 会作为静态前缀拼接在编码最前面。

### Normal 生成器

`Normal` 使用产品 `Pattern` 作为 .NET 日期格式字符串，再通过 `string.Format(..., serial)` 填入流水号。

示例：

```text
yyyyMMdd-{0:0000}
```

在 2026-07-14 第 1 件时生成：

```text
20260714-0001
```

## 运行流程

1. 启动程序。
2. 点击工具栏“设置...”，填写设备目录、可变文本别名、脚踏配置，并维护产品配置。
3. 在“运行设置”中选择产品，点击“保存并应用”。
4. 程序保存 `config.json`，初始化设备，加载所选产品的模板。
5. 主界面显示当前待处理编号、日期和流水号。
6. 根据现场流程点击：

| 操作 | 说明 |
| --- | --- |
| `P 红光预览` | 执行红光预览，不提交流水号。 |
| `M 激光打标` | 执行激光打标。只有 `HS_IsMarkEnd` 返回正常结束时才提交流水号。 |
| `S 已用/跳过` | 操作员确认当前编号已经使用或必须跳过，随后进入下一个编号。 |
| `Q 退出` | 退出程序，当前待确认编号保留，下次启动后继续提示。 |

程序会先把编号写入数据库作为 `Pending` 记录，再调用激光接口。这样即使中途断电，也不会静默地把同一个编号分配给下一件工件。恢复后需要人工检查工件或 MES，再决定重新打标还是确认已用或跳过。

## 历史记录和重新打标

点击工具栏“历史记录”可以按产品和状态查询记录。记录状态包括：

| 状态 | 含义 |
| --- | --- |
| `Pending` | 已占用但未确认完成。 |
| `Marked` | 激光打标正常完成。 |
| `Skipped` | 操作员确认已用或跳过。 |
| `Reprinted` | 历史编号重新打标生成的记录。 |

历史编号重新打标不会占用新的日流水号。重新打标前必须先启动该历史记录所属产品的配置和模板。重新打标正常结束后，会新增一条 `Reprinted` 记录，并通过 `SourceRecordId` 关联原始记录。

## 数据文件

程序运行目录下会产生以下文件：

| 文件 | 说明 |
| --- | --- |
| `config.json` | 运行配置。 |
| `data.db` | SQLite 数据库，保存产品、流水状态和历史记录。 |
| `mark-audit.csv` | 简要审计日志，记录预览、打标、异常和重打标事件。 |

数据库会自动创建，并在启动时补齐缺失列，例如 `Products.CodeGeneratorType`、`Products.SerialStartValue`、`MarkingRecords.SourceRecordId` 等。

## 使用的厂家接口

程序封装使用了以下主要接口：

- `HS_InitialMachine`
- `HS_LoadMarkFile`
- `HS_ChangeTextByNameW`
- `HS_ChangeTextByName`
- `HS_Mark`
- `HS_IsMarkEnd`
- `HS_MarkStop`
- `HS_GetMarkTime`
- `HS_CloseMarkFile`
- `HS_CloseMachine`
- `HS_GetDllVersion`

优先使用 `HS_ChangeTextByNameW` 写入可变文本；如果旧版 DLL 没有该入口，会回退到 `HS_ChangeTextByName`。当前流水号内容建议保持 ASCII，以保证回退接口也能可靠工作。

## 常见问题

### 提示另一个程序在运行

错误码 `1` 通常表示标准打标软件或校正软件仍在占用设备。请关闭相关软件后重新启动本程序。

### 找不到 `HansAdvInterface.dll`

检查 `MachinePath` 是否指向大族激光打标控制软件目录，并确认该目录下存在 `HansAdvInterface.dll`。也可以在设置窗口中点击“读取版本”验证。

### 找不到可变文本别名

检查模板中的可变文本对象别名是否与 `VariableTextAlias` 一致。默认别名为 `CODE`。

### 提示未选择产品

需要先在“产品配置”页保存至少一个产品，再回到“运行设置”页选择该产品并点击“保存并应用”。

### 当天流水号达到 9999

程序会禁止回卷到起始值，避免重复编号。需要现场按生产规则切换日期、产品或人工处理。

## 生产安全

首次测试只使用红光预览，确认文本、位置、尺寸和方向正确后，再在废料或低风险工件上试打。必须保留防护罩、门禁、急停和现场既有激光安全联锁。不要用软件逻辑替代硬件安全回路。
