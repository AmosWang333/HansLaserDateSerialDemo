...
namespace HansLaserDateSerialDemo
{
    ...
    internal sealed class SequenceStore
    {
        ...
        public SequenceStore(string path)
        {
            if (string.IsNullOrWhiteSpace(path))
                throw new ArgumentException("状态文件路径不能为空。", "path");
            _path = path;
        }
        ...
        public Reservation GetOrReserve(DateTime now)
        {
            lock (_gate)
            {
                ...
                string code = CodeGenerator.Build(state.Date, serial);
            ...
            lock (_gate)
            {
                ...
                if (!string.Equals(state.PendingCode, code, StringComparison.Ordinal))
                    throw new InvalidOperationException(
                        "待确认编号不一致。状态文件=" + state.PendingCode + "，程序=" + code);
        ...
        private void SaveAtomic(State state)
        {
            ...
            string[] lines =
            {
                ...
                "PENDING_SERIAL=" + (state.PendingSerial.HasValue
                    ? state.PendingSerial.Value.ToString(CultureInfo.InvariantCulture)
                    : string.Empty),
